...
class Profil extends CI_Controller
{
    ...
    {
        $data['judul'] = 'Daftar Baju';
        $data['user'] = $this->userrole->getBy();
        $data['baju'] = $this->Baju_model->get();
        $data['jlh'] = $this->Keranjang_model->jumlah();
        $this->load->view('layout/header', $data);
        $this->load->view('profil/vw_baju', $data);
    ...
    public function keranjang($id)
    {
        ...
        if ($this->form_validation->run() == false) {
			...
			Baju berhasil ditambahkan ke keranjang!</div>'
            );
    ...
    {
        $jumlah_beli = count($this->input->post('baju'));
        $data_p = [
        ...
        for ($i = 0; $i < $jumlah_beli; $i++) {
            array_push($data_detail, [
                'id_baju' => $this->input->post('baju')[$i],
            ]);
            $data_detail[$i]['no_penjualan'] = $this->input->post(
                'no_penjualan'
            );
        ...
        if (
            $this->Penjualan_model->insert($data_p, $upload_image) &&
            $this->Detail_model->insert($data_detail)
        ) {
            for ($i = 0; $i < $jumlah_beli; $i++) {
                $this->Baju_model->min_stok(
                    $data_detail[$i]['jumlah'],
                    $data_detail[$i]['id_baju']
                ) or die('gagal min stok');
            }
    ...
    public function statusbeli($id)
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->session->set_flashdata(
                'message',
                '<div class="alert alert-success" role="alert">Status Berhasil DiUbah!</div>'
            );
